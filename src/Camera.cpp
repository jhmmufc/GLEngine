...
namespace GameEngine
{
	namespace Rendering
	{
		...
		bool Camera::IsBoxInView(const glm::vec2& position, const glm::vec2& dimensions) const
		{
			...
			const float MIN_DISTANCE_X = dimensions.x / 2.0f + scaledScreenDimensions.x;
			const float MIN_DISTANCE_Y = dimensions.y / 2.0f + scaledScreenDimensions.y;
			...
			float xDepth = MIN_DISTANCE_X - (distVec.x);
			float yDepth = MIN_DISTANCE_Y - (distVec.y);

			// If both the depths are > 0, then we collided
			if ((xDepth > 0 && yDepth > 0) && (xDepth <= m_screenWidth && yDepth <= m_screenHeight)) 
			{
				return true;
			}
